...
// 파라미터의 타입은 string으로 추정
// 반환 타입은 string으로 단언
const myId = getId('hong') as string;

// 타입단언 중첩 (비추)
// 10을 any타입으로 단언한 후에 다시 number타입으로 단언
const num4 = (10 as any) as number;

interface Books {
    shuffle: Function
}
// ?. : optional chaining (null -> undefined, null방지용)
...
// 타입 가드 (type guard)
// 여러 타입 중 하나인 경우에 원하는 타입으로 타입 한정
...
// Bird는 name:string만 가지고 있고
// Mammal은 name:string과 breastfeed:true를 가지고 있어서
// Mammal타입은 Bird타입을 포함할 수 있는 타입
interface Bird {
    name: string;
}
...
// bird는 name만 가지고 있으므로 Mammal 호환타입이 아님
...
// someone이 age를 가지고 있다면(Hong타입 이라면) Hong타입인지를 반환
